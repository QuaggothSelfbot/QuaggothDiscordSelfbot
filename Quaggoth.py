...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Quaggoth Selfbot v{SELFBOT.__version__}] | Loading...')
...
giveaway_Snatcher = config.get('giveaway_Snatcher')
slotbot_Snatcher = config.get('slotbot_Snatcher')
nitro_Snatcher = config.get('nitro_Snatcher')
privnote_Snatcher = config.get('privnote_Snatcher')
...
def startprint():
    if giveaway_Snatcher == True:
        giveaway = "Pog (Enabled)"
    else:
        giveaway = "Not Pog (Disabled)"

    if nitro_Snatcher == True:
        nitro = "Pog (Enabled)"
    else:
        nitro = "Not Pog (Disabled)"

    if slotbot_Snatcher == True:
        slotbot = "Pog (Enabled)"
    else:
        slotbot = "Not Pog (Disabled)"

    if privnote_Snatcher == True:
        privnote = "Pog (Enabled)"
    else:
        privnote = "Not Pog (Disabled)"
...
███  ▀ ███  ███    ███   ███    ███   ███    ███   ███    ███ ███    ███     ███       ███    ███                ▄█    ███   ███    ███ ███▌    ▄   ███          ███    ███ ███    ███     ███
 ▀██████▀▄█ ████████▀    ███    █▀    ████████▀    ████████▀   ▀██████▀     ▄████▀     ███    █▀               ▄████████▀    ██████████ █████▄▄██   ███        ▄█████████▀   ▀██████▀     ▄████▀
                                                                                                                                        ▀
                       {Fore.CYAN}Quaggoth Selfbot {SELFBOT.__version__} | {Fore.WHITE}Logged in as: {Quaggoth.user.name}#{Quaggoth.user.discriminator} {Fore.WHITE}| ID: {Fore.YELLOW}{Quaggoth.user.id}
                       {Fore.CYAN}Privnote Snatcher | {Fore.WHITE}{privnote}
                       {Fore.CYAN}Nitro Snatcher | {Fore.WHITE}{nitro}
...
colorama.init()
Quaggoth = discord.Client()
Quaggoth = commands.Bot(
    description='Quaggoth Selfbot',
    command_prefix=prefix,
    self_bot=True
)
Quaggoth.remove_command('help')
...
async def btc_status():
    ...
    await Quaggoth.change_presence(activity=btc_stream)
...
async def on_message_edit(before, after):
    await Quaggoth.process_commands(after)
...
async def on_message(message):
    ...
    if f'Congratulations <@{Quaggoth.user.id}>' in message.content:
        if giveaway_Snatcher == True:
            if message.author.id == 294882584201003009:
                print(""
    ...
    if 'privnote.com' in message.content:
        if privnote_Snatcher == True:
            ...
            return
    await Quaggoth.process_commands(message)
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Quaggoth Selfbot v{SELFBOT.__version__}] | Logged in as {Quaggoth.user.name}')
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'Quaggoth-Was-Here@Fuckyou.com';
...
print("Weather Command Loaded")

@Quaggoth.command(aliases=['shorteen'])
...
print("Decode Command Loaded")

@Quaggoth.command(name='ebay-view', aliases=['ebay-view-bot', 'ebayviewbot', 'ebayview'])
async def _ebay_view(ctx, url, views: int): # b'\xfc'
    ...
    await ctx.send(embed=em)
print("Ebay-View Command Loaded")

@Quaggoth.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
print("Av Command Loaded")

@Quaggoth.command(aliases=['ri', 'role'])
...
print("Minesweeper Command Loaded")
...
print("Combine Command Loaded")

@Quaggoth.command(name='1337-speak', aliases=['1337speak'])
...
print("1337_speak Command Loaded")

@Quaggoth.command(aliases=['dvwl'])
...
async def blank(ctx): # b'\xfc'
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
          try:
             await Quaggoth.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
          except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
print("Blank Command Loaded")
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
              await Quaggoth.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
print("PFPsteal Command Loaded")

@Quaggoth.command(name='set-pfp', aliases=['setpfp', 'pfpset'])
...
async def dick(ctx, *, user: discord.Member = None): # b'\xfc'
    ...
    await ctx.send(embed=em)
print("Dick Command Loaded")

@Quaggoth.command(aliases=['changehypesquad'])
async def hypesquad(ctx, house): # b'\xfc'
    ...
    try:
        ...
        print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
print("Hypesquad Command Loaded")

@Quaggoth.command(aliases=['tokenfucker', 'disable', 'crash'])
    ...
    while True:
        ...
        while True:
            try:
                ...
                break
print("Tokenfuck Command Loaded")

@Quaggoth.command()
async def masslogin(ctx, choice = None): # b'\xfc'
    await ctx.message.delete()
    _masslogin(choice)
print("Masslogin Command Loaded")
...
print("Fakenet Command Loaded")

@Quaggoth.command(aliases=['tokinfo', 'tdox'])
async def tokeninfo(ctx, _token): # b'\xfc'
    ...
    return await ctx.send(embed=em)
print("Tokeninfo Command Loaded")

@Quaggoth.command()
async def copy(ctx): # b'\xfc'
    await ctx.message.delete()
    await Quaggoth.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Quaggoth.guilds:
...
async def masschannel(ctx): # b'\xfc'
    ...
    for _i in range(250):
        try:
            ...
            return
print("Masschannel Command Loaded")
...
async def delchannels(ctx): # b'\xfc'
    ...
    for channel in list(ctx.guild.channels):
        try:
            ...
            return
print("Delchannels Command Loaded")
...
async def massunban(ctx): # b'\xfc'
    ...
    for users in banlist:
        try:
            ...
            pass
print("Massunban Command Loaded")
...
async def dm(ctx, user : discord.Member, *, message): # b'\xfc'
    ...
    if ctx.author.id == Quaggoth.user.id:
        ...
        try:
            ...
            pass
print("DM Command Loaded")

@Quaggoth.command(name='get-color', aliases=['color', 'colour', 'sc'])
...
print("Get_Color Command Loaded")
...
async def tinyurl(ctx, *, link): # b'\xfc'
    ...
    await ctx.send(embed=em)
print("Tinyurl Command Loaded")

@Quaggoth.command(aliases=['rainbow-role'])
async def rainbow(ctx, *, role): # b'\xfc'
    ...
    while True:
        try:
            ...
            break
print("Rainbow Command Loaded")

@Quaggoth.command(name='8ball')
...
print("8ball Command Loaded")

@Quaggoth.command(aliases=['slots', 'bet'])
...
print("Joke Command Loaded")

@Quaggoth.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1
            channel = Quaggoth.get_channel(int(channelid))
            await channel.send('!d bump')
            print(f'{Fore.MAGENTA}[AUTO-BUMP] {Fore.GREEN}Bump number: {count} sent'+Fore.RESET)
            await asyncio.sleep(7200)
        except Exception as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
print("Auto-bump Command Loaded")
...
async def upper(ctx, *, message): # b'\xfc'
    ...
    await ctx.send(message)
print("Upper Command Loaded")

@Quaggoth.command(aliases=['guildpfp'])
async def guildicon(ctx): # b'\xfc'
    await ctx.message.delete()
    em = discord.Embed(title=ctx.guild.name)
    em.set_image(url=ctx.guild.icon_url)
    await ctx.send(embed=em)
print("Guildicon Command Loaded")

@Quaggoth.command(name='backup-f', aliases=['friendbackup', 'friend-backup', 'backup-friends', 'backupf'])
async def _backup_f(ctx): # b'\xfc'
    await ctx.message.delete()
    for friend in Quaggoth.user.friends:
       friendlist = (friend.name)+'#'+(friend.discriminator)
       with open('Backup/Friends.txt', 'a+') as f:
           f.write(friendlist+"\n" )
    for block in Quaggoth.user.blocked:
        blocklist = (block.name)+'#'+(block.discriminator)
        with open('Backup/Blocked.txt', 'a+') as f:
            f.write(blocklist+"\n" )
print("Backup Command Loaded")

@Quaggoth.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
async def _first_message(ctx, channel: discord.TextChannel = None): # b'\xfc'
    ...
    await ctx.send(embed=embed)
print("First-message Command Loaded")
...
async def abc(ctx): # b'\xfc'
    ...
    for _next in ABC[1:]:
        ...
        await asyncio.sleep(2)
print("ABC Command Loaded")

@Quaggoth.command(aliases=['bitcoin'])
async def btc(ctx): # b'\xfc'
    ...
    await ctx.send(embed=em)
print("BTC Command Loaded")

@Quaggoth.command(aliases=['ethereum'])
...
print("Topic Command Loaded")

@Quaggoth.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
async def hentai(ctx): # b'\xfc'
    ...
    res = r.json()
    em=discord.Embed(title=" ", description="Does it trigger you?", color=0x9900ff)
    em.set_author(name="Hentai", url="https://steamuserimages-a.akamaihd.net/ugc/777351336260394515/216194867752C5337642CB0C3E0E065F1B061008/", icon_url="https://steamuserimages-a.akamaihd.net/ugc/777351336260394515/216194867752C5337642CB0C3E0E065F1B061008/")
    em.set_image(url=res['url'])
    em.set_footer(text="GIFs are random.")
    await ctx.send(embed=em)
print("Hentai Command Loaded")
...
async def kiss(ctx, user: discord.Member): # b'\xfc'
    ...
    em.set_image(url=res['url'])
    await ctx.send(embed=em)
print("Kiss Command Loaded")

@Quaggoth.command(aliases=['proxy'])
...
print("Uptime Command Loaded")

@Quaggoth.command(aliases=['purge'])
async def delmsg(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Quaggoth.user).map(lambda m: m):
        try:
...
@Quaggoth.command(name='group-leaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def _group_leaver(ctx): # b'\xfc'
    await ctx.message.delete()
    for channel in Quaggoth.private_channels:
        if isinstance(channel, discord.GroupChannel):
            await channel.leave()
print("Group_leaver Command Loaded")
...
async def stream(ctx, *, message): # b'\xfc'
    ...
    await Quaggoth.change_presence(activity=stream)
print("Stream Command Loaded")
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await Quaggoth.change_presence(activity=game)
print("Game Command Loaded")

@Quaggoth.command()
async def listening(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await Quaggoth.change_presence(
        activity=discord.Activity(
            type=discord.ActivityType.listening,
            name=message,
        ))
print("Listening Command Loaded")
...
print("Watching Command Loaded")

@Quaggoth.command(aliases=['markasread', 'ack'])
async def read(ctx): # b'\xfc'
    await ctx.message.delete()
    for guild in Quaggoth.guilds:
        await guild.ack()
print("Read Command Loaded")
...
print("Tableflip Command Loaded")
...
print("Unflip Command Loaded")

@Quaggoth.command()
async def bold(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await ctx.send('**'+message+'**')
print("Bold Command Loaded")

@Quaggoth.command()
async def secret(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await ctx.send('||'+message+'||')
print("Secret Command Loaded")

@Quaggoth.command(name='role-hexcode', aliases=['rolecolor'])
async def _role_hexcode(ctx, *, role: discord.Role): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(f"{role.name} : {role.color}")
print("Rolecolor Command Loaded")

@Quaggoth.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
async def _get_hwid(ctx): # b'\xfc'
    await ctx.message.delete()
    print(f"HWID: {Fore.YELLOW}{hwid}"+Fore.RESET)
print("HWID Command Loaded")

@Quaggoth.command()
async def empty(ctx): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(chr(173))
print("Empty Command Loaded")

@Quaggoth.command()
async def everyone(ctx): # b'\xfc'
    await ctx.message.delete()
    await ctx.send('https://@everyone@google.com')
print("Everyone Command Loaded")

@Quaggoth.command()
async def logout(ctx): # b'\xfc'
    await ctx.message.delete()
    await Quaggoth.logout()
print("Logout Command Loaded")

@Quaggoth.command(aliases=['btc-stream', 'streambtc', 'stream-btc', 'btcstatus'])
async def btcstream(ctx):  # b'\xfc'
    await ctx.message.delete()
    btc_status.start()
print("BTCstream Command Loaded")

@Quaggoth.command(name='steal-all-pfp', aliases=['steal-all-pfps', 'stealallpfps'])
async def _steal_all_pfp(ctx): # b'\xfc'
    await ctx.message.delete()
    Dump(ctx)
print("StealAllPFP Command Loaded")

@Quaggoth.command(aliases=['clearconsole', 'consoleclear'])
async def cls(ctx): # b'\xfc'
    await ctx.message.delete()
    Clear()
    startprint()
print("CLS Command Loaded")

@Quaggoth.command()
async def nitro(ctx): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(Nitro())
print("Nitro Command Loaded")

@Quaggoth.command(name='gmail-bomb', aliases=['gmail-bomber', 'gmailbomb', 'email-bomber', 'emailbomber'])
async def _gmail_bomb(ctx): # b'\xfc'
    await ctx.message.delete()
    GmailBomber()
print("Gmail-Bomb Command Loaded")

@Quaggoth.command()
async def deletos(ctx): # b'\xfc'
    await ctx.message.delete()
    embed = discord.Embed()
    embed.description = "Delet that right now"
    embed.set_image(url="https://emoji.gg/assets/emoji/1936_deletos.png")
    await ctx.send(embed=embed)
print("Deletos Command Loaded")
